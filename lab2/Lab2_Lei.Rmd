...
```{r}
m6.3 <- lm(Revenue ~ Planned.revenue, data=data6.2)
confint(m6.3)
```

The confidence interval of planned revenue coefficient is $(0.969, 0.970)$, thus we can reject the null hypothesis that the average revenue is not different from 95 cents when the planned revenue increases by $1.
