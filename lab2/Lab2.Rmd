---
title: "W271 Lab2"
author: "Lei Yang, Subhashini Raghunathan, Ron Cordell"
...
pairs(wage~education+experience+age, data=df1, upper.panel=panel.smooth, lower.panel=panel.cor, diag.panel=panel.hist, main='wage, education, experience, age')
```

```{r echo=FALSE}
pairs(logWage~experienceSquare+IQscore+dad_education+mom_education, data=df1, upper.panel=panel.smooth, lower.panel=panel.cor, diag.panel=panel.hist, main='logWage, experience2, IQ, parent education')
```
...
The NA observations force the throwing out of the corresponding rows of the data frame, significantly reducing the power of the model. In addition, from the boxplot below we can see the variation of these two variables is not quite large, we would apply some rule to interpolate the missing values.


```{r}
boxplot(df1[,c('mom_education','dad_education')])
```
...
stargazer(model4.3, model4.4, model4.5, model4.5.na, m4.5.4,
          df=F, title='Results',
...
```
```{r echo=FALSE}
pairs(voteshare~absolute_wealth + urb + lit, data=df2, upper.panel=panel.smooth, lower.panel=panel.cor, diag.panel=panel.hist, main='shared vote, wealth, urban, literate')
```


We can see that _urb_ and _absolute_wealth_ have issues with distribution and are highly skewed. Creating a new variable, _logUrb_, as the log(urb) does help with the distribution. However, _absolute_wealth_ doesn't get as much help from this treatment.

Examining the _absolute_wealth_ variable we can see there are a large number of 2.0e+00 values, which is also shown as the minimum value in the summary.
...
There is a linear relationship between _logWealth_ and _voteshare_, statistically significant at the .002 level but the practical significance is small.
...
```{r}
model5.1.3 <- lm(voteshare ~ logWealth*factor(region), data = df2)
summary(model5.1.3)
...
### Variable Analysis Matrix Plots
...
pairs(Revenue ~ Product.cost + Quantity + Unit.cost + Unit.price, 
      data=data6.2, 
      upper.panel=panel.smooth, 
      lower.panel=panel.cor, 
      diag.panel=panel.hist, 
      main='Revenue, Product Cost, Quantity, Unit Cost, Unit Price')
```
It appears the distribution of the ratio variables is heavily skewed thus we check the relationships after performing log transformations.
```{r echo=FALSE, fig.width=6, fig.height=8}
# it appears the distribution of the ratio variables is heavily skewed
# thus we check relations with log value, which has better normality in distribution
pairs(log(Revenue)~ log(Product.cost) + 
        log(Quantity) + log(Unit.cost) + log(Unit.price), 
      data=data6.2, 
      upper.panel=panel.smooth, 
      lower.panel=panel.cor, 
      diag.panel=panel.hist, 
...
### Conditions and Unusual Observations

There are many NAs in the data as a result of countries not carrying certain products or because some countries didn't start selling product until 2005.

The 2007 data appears to be for about 1/2 year and not a full year. Since the data is by year this will require caution when using the 2007 as test data; we will restrict testing to be against 2006 complete year data.

Golf equipment is the most profitable product category; mountaineering equipment the least.

Web sales have become the dominant sales channel in all countries but a couple that still have measurable sales in other channels like email and fax. Sales channels in countries that have no significant revenue by that channel can be dropped as well as the related overhead costs.
...
We have identified two approaches to regression model evaluation: significance and Log Mean Squared prediction error. In both approaches the models are approached the same: start with a parsimonious model and build upon it, testing for significance or prediction error. Significance is measured as estimating model parameters from years 2004-2005 of the data, and estimating a second set of model parameters from 2006-2007 model data. Measure the statistical different between the estimates. 

Log Mean Square Error is measured by estimating model parameters from 2004-2005 data and then using that model to predict the 2006 data. 2007 data is left out of the prediction test because it is an incomplete year's worth of data. The square root of the sum of the squared differences of the log predicted values is used as a comparison to measure error. LMSE was chosen over Root Mean Squared Error because log transformations have been taken of the prediction variables already.

We investigate the interactions of Country, Year, Cost by Product line and planned revenue.

## Model Predictions and LMSE
...
The OLS model parameters may be biased if the estimates are not linearly related to the dependent variable, revenue. Endogeneity bias is another source of bias - factors such as business law, culture, relative economic strength are all examples of how estimates can be biased. While country is taken into account in the model, broad cultural influences that transcend country boundaries are not.

### Instrumental Variable Approach

An IV approach that could improve the forecasting model is to take into account cultural norms that favor some products over others. For example, an IV variable to guage golf products would be the number of golf courses in a country, or a golf courses per capita value.