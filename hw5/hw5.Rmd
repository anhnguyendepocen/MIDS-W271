...
```{r}
data<-read.csv("googl.csv", header=T)
plot(as.Date(data[,1], "%d-%b-%y"), data$Close, type="l", xlab="Date", ylab="Close Price ($)", col="blue", main="GOOG Stock Price")
```

For Google, the price stays stable before mid July 2015, before have a big jump and steadily increase through January 2016, followed by some fluctuation after that.

**Example 2**: control parameters in a semiconductor process. Various sensors are recording time series data during the process, a feedback controller is adjusting setpoint in real time to ensure a stable chamber condition during the process.

**Example 3**: biometric data. With various wearables becoming popular, more biometric data are collected from daily life, including heartrate, sleep quality, activities, food intake, etc. Overtime those data yield different time series to indicate one's life style, healthness, and hopefully can be further utilized to predict or diagnose potential health problem.
